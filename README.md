# Hi, I'm Toto

📍 **Tokyo ↔ Tbilisi ↔ Rome** | 🛡️ **Staff Engineer at Kape/ExpressVPN** | 🧠 **Building tools for thought**

![Swift](https://img.shields.io/badge/-Swift-FA7343?style=flat-square&logo=swift&logoColor=white)
![Rust](https://img.shields.io/badge/-Rust-000000?style=flat-square&logo=rust&logoColor=white)
![Claude](https://img.shields.io/badge/-Claude-000000?style=flat-square&logo=anthropic&logoColor=white)
![MCP](https://img.shields.io/badge/-MCP-FF6B6B?style=flat-square&logo=protocol&logoColor=white)
![iOS](https://img.shields.io/badge/-iOS-000000?style=flat-square&logo=ios&logoColor=white)
![macOS](https://img.shields.io/badge/-macOS-000000?style=flat-square&logo=apple&logoColor=white)
![SwiftUI](https://img.shields.io/badge/-SwiftUI-0062D3?style=flat-square&logo=swift&logoColor=white)
![SQLite](https://img.shields.io/badge/-SQLite-003B57?style=flat-square&logo=sqlite&logoColor=white)

> Building humane tools for knowledge work. Researching file-native systems and AI-augmented thinking. Learning Rust for high-performance tooling.

## 🚀 Current Focus

* [Plaintext Commons](https://github.com/totocaster/plaintext-commons) — A file-native thinking system where plain Markdown files are the source of truth. Not apps that control your files, but files you control. Combines AI agents, fast indexing, and human-centered design.
* [Arrowhead](https://github.com/totocaster/arrowhead) — Fast search and indexing for knowledge bases. Built in Rust with both full-text and semantic search capabilities. Features CLI/MCP integration for AI agents.

### 📱 Professional Work

**Staff Engineer** at **Kape Technologies/ExpressVPN** - Building privacy tools for millions of users on Apple platforms. Extensive experience with SwiftUI, UIKit, and system-level iOS development.

## 🌱 Currently Learning

- **Rust** - Building high-performance systems and CLI tools
- **AI/ML** - Deep learning with PyTorch, LLM fine-tuning, and AI-augmented workflows
- **Knowledge Systems** - Researching tools for thought and human-computer collaboration

## 📊 GitHub Activity

![GitHub Contribution Graph](https://ghchart.rshah.org/totocaster)

## 💭 Philosophy

I believe in:
- **File over app** - Your data should outlive any application
- **Plain text** - The most durable, portable format
- **Open protocols** - Standards that empower users, not vendors
- **Humane design** - Technology that respects human cognition and creativity

## 🔗 Connect

[![Twitter](https://img.shields.io/badge/-@totocaster-1DA1F2?style=flat-square&logo=twitter&logoColor=white)](https://twitter.com/totocaster)
[![Mastodon](https://img.shields.io/badge/-@ttt-6364FF?style=flat-square&logo=mastodon&logoColor=white)](https://mastodon.social/@ttt)
[![Bluesky](https://img.shields.io/badge/-@ttvl.co-00A8E8?style=flat-square&logo=bluesky&logoColor=white)](https://bsky.app/profile/ttvl.co)
[![Website](https://img.shields.io/badge/-ttvl.co-FF5722?style=flat-square&logo=hugo&logoColor=white)](https://ttvl.co)
[![LinkedIn](https://img.shields.io/badge/-Toto_Tvalavadze-0077B5?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/totocaster)
[![GitHub](https://img.shields.io/badge/-Follow-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/totocaster)
